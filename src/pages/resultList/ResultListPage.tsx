...
export const ResultListPage = () => {
    const { Option } = Select;
    const subjectList = useReduxSelector((s) => s.subjectList.subjectList);
                    ...
                    <Row gutter={30}>
                        <Col span={8}>
                            <Form.Item
                                ...
                                label="Subject"
                            >
                                <Select placeholder="Choose Your Subject" onSelect={setSubject}>
                                    {subjectList?.map((i) => <Option value={i._id}
                                                                     key={i._id}>{i.subjectName}</Option>)}
                                </Select>
                            </Form.Item>
                        </Col>
                        <Col span={8}>
                            <Form.Item
                                ...
                                label="Assignment"
                            >
                                <Select onSelect={choseAssignment} placeholder="Choose Your Assignment"
                                        disabled={showAssignment}>
                                    {Assignment?.map((i) => <Option value={i._id}
                                                                    key={i._id}>{i.assignmentName}</Option>)}
                                </Select>
                            </Form.Item>
                        </Col>
                        <Col span={8}>
